...
class BertEmbeddingDataset(CustomDataset):
    ...
    all_words_path ='cache/fasttext/nguyennb/all_words.txt', mode = 'mean'):
        ...
        for i, sample in enumerate(self.data):
            ...
            if mode == 'first':
                torch.cat(
                    (sample, this_sent_embedded_first.reshape(1,second_dim_num,-1)),
                    dim=2
                )
            elif mode == 'mean':
                torch.cat(
                    (sample, this_sent_embedded_mean.reshape(1,second_dim_num,-1)),
                    dim=2
                )
            elif mode == 'last':
                torch.cat(
                    (sample, this_sent_embedded_last.reshape(1,second_dim_num,-1)),
                    dim=2
                )
