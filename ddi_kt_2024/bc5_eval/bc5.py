...
def evaluate_bc5(answer, verbose=False):
    ...
    intra = []
    with open('ddi_kt_2024/bc5_eval/eval/bc5_intra.txt') as f:
        for l in f:
    ...
    f = open('ddi_kt_2024/bc5_eval/eval/bc5_evaluate.txt')
    for line in f:
...
def evaluate_bc5_intra(eval_data):
    ...
    f = open('ddi_kt_2024/bc5_eval/eval/bc5_evaluate_intra.txt')
    for line in f:
        pmid, rel, e1, e2 = line.strip().split()
        if rel.endswith('(r)'):
